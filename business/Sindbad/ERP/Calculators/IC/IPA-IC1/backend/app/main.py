...
from app.preview import compute_preview as _compute_preview

@app.post("/compute/preview")
async def compute_preview_endpoint(body: dict = Body(...)):
    # dev-only public access (align with other public routes)
    if os.getenv("DEV_ALLOW_PUBLIC_COMPUTE") != "1":
        # in prod you can swap to auth and remove this guard
        raise HTTPException(status_code=404, detail="Not found")
    return _compute_preview(body)
